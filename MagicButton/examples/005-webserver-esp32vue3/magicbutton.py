...
class MagicButton:
    ...
    def autoshutdowntimer(self,tim):
        ...
        if self.powerstate>self.powershutdown_time_s:
            if self.powerstate%10==0:
                print("uptime "+ str(self.powershutdown_time_s) +",shutdown now ,"+str(self.powerstate))
            self.powerpin.value(0) # 定时时间到了之后，自动关机
    ...
    def get_shutdown_time(self):
        return self.powershutdown_time_s
    def set_shutdown_time(self,seconds):
        self.powershutdown_time_s = seconds
    # led 控制
